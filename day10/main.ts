...
function printGrid(positions: Set<string>, xRange: Vec2D, yRange: Vec2D) {
...
function day10(input: string) {
    ...
    let regex = /^position=<(.+), (.+)> velocity=<(.+), (.+)>$/;

    let points: Point[] = new Array(rows.length);

    rows.forEach((r, i) => {
        let res = regex.exec(r);
        points[i] = {
            position: { x: parseInt(res[1], 10), y: parseInt(res[2], 10) },
            velocity: { x: parseInt(res[3], 10), y: parseInt(res[4], 10) }
        };
    });

    let minX = Math.min(...points.map((p) => p.position.x));
    let minY = Math.min(...points.map((p) => p.position.y));
    let maxX = Math.max(...points.map((p) => p.position.x));
    let maxY = Math.max(...points.map((p) => p.position.y));

    let currentPositions = new Set<string>(points.map((p) => `${p.position.x}|${p.position.y}`));
    ...
    while (true) {
        ...
        let newMinX = Math.min(...points.map((p) => p.position.x));
        let newMinY = Math.min(...points.map((p) => p.position.y));
        let newMaxX = Math.max(...points.map((p) => p.position.x));
        let newMaxY = Math.max(...points.map((p) => p.position.y));
        
        if (newMinX < minX || newMinY < minY || newMaxX > maxX || newMaxY > maxY) {
            printGrid(currentPositions, { x: minX, y: maxX}, { x: minY, y: maxY });
            console.log(second);
            break;
        } else {
            currentPositions = new Set<string>(points.map((p) => `${p.position.x}|${p.position.y}`));
        }
...
day10(`position=< 9,  1> velocity=< 0,  2>
...
day10(`position=<-31684, -53051> velocity=< 3,  5>
