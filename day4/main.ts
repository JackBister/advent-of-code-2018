...
function timeStampToInt(timeStamp: string) {
    return parseInt(timeStamp.slice(3), 10);
}

function day4(input: string) {
    let strings = input.split('\n');

    // Map from guard ID to array of length 60 counting number of times the guard was asleep that minute
    let sleepyTimes = new Map<string, number[]>();

    let dateTimeRegex = /\[(\d\d\d\d-\d\d-\d\d \d\d:\d\d)\]/
    let sorted = strings.sort((a, b) => {
        let dateA = new Date(dateTimeRegex.exec(a)[1]);
        let dateB = new Date(dateTimeRegex.exec(b)[1]);

        return dateA.valueOf() - dateB.valueOf();
    });

    let fullRegex = /^\[\d\d\d\d-\d\d-\d\d (\d\d:\d\d)\] (.*)$/;
    let beginsShiftRegex = /^Guard #(\d+) begins shift/;

    let currentGuard: string = null;
    let currentSleepStartTime: number = null;
    sorted.forEach((s) => {
        let matches = fullRegex.exec(s);
        let timeStamp = matches[1];
        let eventString = matches[2];

        let beginsShift = beginsShiftRegex.exec(eventString);
        if (beginsShift) {
            currentGuard = beginsShift[1];
        } else if (eventString === 'falls asleep') {
            currentSleepStartTime = timeStampToInt(timeStamp);
        } else if (eventString === 'wakes up') {
            let sleepEndTime = timeStampToInt(timeStamp);
            let sleepMinutes = sleepyTimes.has(currentGuard) ? sleepyTimes.get(currentGuard) : new Array(60).fill(0);
            for (let i = currentSleepStartTime; i < sleepEndTime; ++i) {
                sleepMinutes[i]++;
            }
            sleepyTimes.set(currentGuard, sleepMinutes);
        }
    });

    let mostSleepy: string = null;
    let mostSleepyMinute: number = null;
    let maxSleepTime = -1;

    let partTwo = new Map<string, [number, number]>();

    for (let sleep of sleepyTimes.entries()) {
        let s = sum(sleep[1]);
        let max = Math.max(...sleep[1]);
        let maxIndex = sleep[1].indexOf(max);
        partTwo.set(sleep[0], [max, maxIndex]);
        if (s > maxSleepTime) {
            mostSleepy = sleep[0];
            mostSleepyMinute = maxIndex;
            maxSleepTime = s;
        }
    }

    let mostSleepyPartTwo: string = null;
    let mostSleptPartTwo: number = -1;
    for (let p2 of partTwo.entries()) {
        if (p2[1][0] > mostSleptPartTwo) {
            mostSleepyPartTwo = p2[0];
            mostSleptPartTwo = p2[1][0];
        }
    }

    console.log(parseInt(mostSleepy, 10) * mostSleepyMinute);
    console.log(parseInt(mostSleepyPartTwo, 10) * partTwo.get(mostSleepyPartTwo)[1]);
}

day4(`[1518-11-01 00:00] Guard #10 begins shift
...
day4(`[1518-05-30 00:04] Guard #2417 begins shift
